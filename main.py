...
def cmd_start(user, chat_id, arg):
    ...
    if arg:
        ...
        if get_setting("delete_enabled", False) and sent_ids:
            ...
            if delay_min:
                schedule_countdown(chat_id, sent_ids, arg, user, delay_min)
...
def build_commands_text(user):
    text = "📚 <b>Commandes</b>\n\n"
    text += (
        "<blockquote>👥 <b>Utilisateurs</b>\n"
        "/start • démarrer le bot\n"
        "/help • afficher ce menu</blockquote>"
    )
    if is_admin(user.id):
        text += (
            "\n\n<blockquote>🛡️ <b>Administration</b>\n"
            ...
            "/import • restaurer une sauvegarde</blockquote>"
        )
    return text
...
def build_stats_screen():
    ...
    text = (
        "📊 <b>Statistiques</b>\n\n"
        f"<blockquote>👥 <b>{total_users}</b> utilisateur(s) au total</blockquote>\n\n"
...
def cmd_placeholders(message):
    ...
    text = (
        ...
        "<blockquote>👋 <b>{mention}</b> • identifie l'utilisateur par son nom complet\n"
        ...
        "🔢 <b>{count}</b> • nombre de fichiers envoyés <i>(message de suppression)</i></blockquote>\n\n"
...
def cmd_batch(message):
    ...
    bot.reply_to(message, "📦 Transfère-moi le PREMIER fichier de la plage (depuis le canal source).")
...
def resolve_batch_range(admin_chat_id, channel_id, start, end):
    """Parcourt tous les messages entre `start` et `end` (inclus) dans `channel_id`,
    et récupère leur file_id permanent un par un via un forward temporaire (supprimé aussitôt)."""
    items = []
    for mid in range(start, end + 1):
        try:
            fwd = bot.forward_message(admin_chat_id, channel_id, mid)
        except Exception:
            continue
...
def handle_pending(message):
    ...
    # ---------- batch (plage : premier fichier -> dernier fichier) ----------
    if action == "batch_first":
        chat_id2, msg_id2 = get_forward_channel_msgid(message)
        if not chat_id2:
            bot.reply_to(message, "❌ Ce n'est pas un message transféré depuis un canal. Réessaie, ou /cancel.")
            return True
        state["data"] = {"channel_id": chat_id2, "first": msg_id2}
        state["action"] = "batch_last"
        bot.reply_to(message, "📦 Reçu. Transfère-moi maintenant le DERNIER fichier de la plage (même canal).")
        return True

    if action == "batch_last":
        chat_id2, msg_id2 = get_forward_channel_msgid(message)
        channel_id = state["data"]["channel_id"]
        if not chat_id2 or chat_id2 != channel_id:
            bot.reply_to(message, "❌ Ce message ne vient pas du même canal que le premier fichier. Réessaie, ou /cancel.")
            return True
        ...
        items = resolve_batch_range(message.chat.id, channel_id, start, end)
        if not items:
...
if __name__ == "__main__":
    threading.Thread(target=countdown_scheduler_loop, daemon=True).start()
