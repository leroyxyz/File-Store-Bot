...
def build_mention(user):
    """Tague toujours par nom complet, jamais par @pseudo."""
...
def send_rich_message(chat_id, content, ctx=None, reply_markup=None):
    """Envoie un message personnalisé (accueil/suppression/broadcast) avec repli sécurisé.
    Si la mise en forme avancée échoue à l'envoi, on retente en texte brut plutôt que de rien envoyer."""
...
def send_commands_menu(chat_id, user, edit_message_id=None):
    ...
    kb = types.InlineKeyboardMarkup()
    if is_admin(user.id):
        kb.row(types.InlineKeyboardButton("➤ Réglages", callback_data="menu_admin"))
    kb.row(
    ...
    if edit_message_id:
        try:
            bot.edit_message_text(text, chat_id, edit_message_id, parse_mode="HTML", reply_markup=kb)
            return
        except Exception:
            pass
...
def build_admin_menu():
    ...
    kb.row(types.InlineKeyboardButton("➤ Contenu", callback_data="menu_content"))
...
def build_content_menu():
    ...
    kb.row(
        ...
        types.InlineKeyboardButton("◆ Aperçu", callback_data="preview_welcome"),
    )
    kb.row(
        types.InlineKeyboardButton("✎ Modifier la suppression", callback_data="edit_delete"),
        types.InlineKeyboardButton("◆ Aperçu", callback_data="preview_delete"),
    )
    kb.row(
        types.InlineKeyboardButton("◄ Retour", callback_data="menu_admin"),
        types.InlineKeyboardButton("✕ Fermer", callback_data="close_menu"),
...
def build_stats_screen():
    ...
    text = (
        ...
        f"Force-sub — <b>{fs_state}</b>\n"
        f"Protection — <b>{protect_state}</b>\n"
        f"Auto-suppression — <b>{del_state}</b>"
    )
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("◄ Retour", callback_data="menu_admin"),
...
def cmd_placeholders(message):
    ...
    text = (
        ...
        "<b>{time}</b> — délai avant suppression <i>(message de suppression)</i>\n"
...
def cmd_setwelcome(message):
    ...
    bot.reply_to(message, "Envoie le nouveau message d'accueil (texte, image, mise en forme). Astuce : /placeholders")
...
def cmd_setdelete(message):
    ...
    bot.reply_to(message, "Envoie le message affiché après l'envoi de tous les fichiers d'un lien. Astuce : /placeholders")
...
def cmd_previewwelcome(message):
    ...
    welcome = get_setting("welcome")
    if not welcome:
        bot.reply_to(message, "Aucun message d'accueil configuré pour l'instant.")
        return
...
def cmd_previewdelete(message):
    ...
    del_cfg = get_setting("delete_msg")
    if not del_cfg:
        bot.reply_to(message, "Aucun message de suppression configuré pour l'instant.")
        return
...
def cmd_cancel(message):
    if message.from_user.id in pending:
        ...
        bot.reply_to(message, "Aucune opération en cours.")
...
def handle_pending(message):
    ...
    if action == "setwelcome":
        ...
        msg = "<b>Accueil mis à jour.</b>"
        if unknown:
            msg += f"\n<i>Variable(s) inconnue(s) ignorée(s) :</i> {', '.join('{' + u + '}' for u in unknown)}"
    ...
    if action == "setdelete_msg":
        ...
        bot.reply_to(message, "Après combien de minutes les fichiers doivent-ils être supprimés ?")
        return True
    ...
    if action == "setdelete_time":
        ...
        msg = f"<b>Message et délai enregistrés</b> ({minutes} min)."
        if unknown:
            msg += f"\n<i>Variable(s) inconnue(s) ignorée(s) :</i> {', '.join('{' + u + '}' for u in unknown)}"
        if not get_setting("delete_enabled", False):
            msg += "\n<i>Rappel : la suppression automatique est actuellement désactivée.</i>"
        bot.reply_to(message, msg, parse_mode="HTML")
        return True
    ...
    if action == "fs_add_channel":
        ...
        if any(c["channel_id"] == chat_id2 for c in fs_existing.get("channels", [])):
            pending.pop(uid)
            bot.reply_to(message, "Ce canal est déjà dans la liste.")
            return True
        ...
        try:
            ...
            bot.reply_to(
                message,
                "Impossible de générer un lien automatiquement (le bot doit être admin de ce canal avec le "
                "droit « Inviter des utilisateurs via un lien »).\n\n"
                "Envoie-moi manuellement le lien d'invitation public de ce canal (https://t.me/...)."
            )
            return True
        ...
        bot.send_message(message.chat.id, f"<b>Canal ajouté</b> — {title}", parse_mode="HTML", reply_markup=kb)
        return True

    if action == "fs_add_link":
        link = (message.text or "").strip()
        if not link.startswith("http"):
            bot.reply_to(message, "Ce n'est pas un lien valide. Réessaie.")
            return True
    ...
    if action == "broadcast_content":
        ...
        bot.reply_to(
            ...
            "Envoie-le sous la forme : Texte - https://lien.com\n"
            "Ou tape /skip pour diffuser sans bouton."
        )
        return True
    ...
    if action == "broadcast_button":
        ...
        if raw and raw.lower() != "/skip":
            if " - http" in raw:
                ...
                if url.startswith("http"):
                    ...
                    bot.reply_to(message, "Lien invalide, diffusion envoyée sans bouton.")
            else:
                bot.reply_to(message, "Format non reconnu, diffusion envoyée sans bouton.")
        ...
        for target in user_ids:
            try:
                ...
                failed += 1
        bot.reply_to(message, f"<b>Diffusion terminée.</b> Envoyés : {sent} — Échecs : {failed}", parse_mode="HTML")
        return True
...
def callback_router(call):
    ...
    if data == "fs_toggle":
        ...
        if not fs.get("channels"):
            bot.answer_callback_query(call.id)
            pending[user.id] = {"action": "fs_add_channel"}
            bot.send_message(chat_id, "Transfère un message du canal à rendre obligatoire (le bot doit déjà en être admin).")
            return
    ...
    if data == "fs_add_channel_start":
        bot.answer_callback_query(call.id)
        pending[user.id] = {"action": "fs_add_channel"}
        bot.send_message(chat_id, "Transfère un message du canal à ajouter (le bot doit déjà en être admin).")
        return
    ...
    if data.startswith("fs_remove:"):
        ...
        if not channels:
            fs["enabled"] = False
        set_setting("force_sub", fs)
        bot.answer_callback_query(call.id, "Canal retiré.")
        show_menu(chat_id, build_channels_menu, msg_id)
        return
    ...
    if data == "edit_welcome":
        ...
        bot.send_message(chat_id, "Envoie le nouveau message d'accueil (texte, image, mise en forme).")
        return
    ...
    if data == "preview_welcome":
        ...
        welcome = get_setting("welcome")
        if not welcome:
            bot.send_message(chat_id, "<i>Aucun message d'accueil configuré.</i>", parse_mode="HTML")
        else:
            send_rich_message(chat_id, welcome, ctx=build_ctx(user))
        return

    if data == "edit_delete":
        bot.answer_callback_query(call.id)
        pending[user.id] = {"action": "setdelete_msg"}
        bot.send_message(chat_id, "Envoie le message affiché après l'envoi de tous les fichiers d'un lien.")
        return
    ...
    if data == "preview_delete":
        ...
        del_cfg = get_setting("delete_msg")
        if not del_cfg:
            bot.send_message(chat_id, "<i>Aucun message de suppression configuré.</i>", parse_mode="HTML")
        else:
    ...
    if data == "new_broadcast":
        bot.answer_callback_query(call.id)
        pending[user.id] = {"action": "broadcast_content"}
        bot.send_message(chat_id, "Envoie le texte/image à diffuser à tous les utilisateurs.")
        return
