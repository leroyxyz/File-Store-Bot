...
def back_to_content_kb(key=None):
    ...
    kb.row(types.InlineKeyboardButton("◄ Retour", callback_data="menu_content"))
    return kb
...
def countdown_line(remaining_seconds):
    remaining_seconds = max(0, remaining_seconds)
    h, rem = divmod(remaining_seconds, 3600)
    m, s = divmod(rem, 60)
    if h:
        return f"⏳ Suppression dans : {h:02d}:{m:02d}:{s:02d}"
    return f"⏳ Suppression dans : {m:02d}:{s:02d}"
...
def run_countdown_and_delete(chat_id, sent_ids, code, user, total_minutes):
    del_cfg = get_setting("delete_msg") or empty_content()
    ctx = build_ctx(user, delay=total_minutes, count=len(sent_ids))
    base_text, entities = render_entities(del_cfg.get("text") or "", del_cfg.get("entities"), ctx)
    total_seconds = int(total_minutes * 60)
    full_text = (base_text + "\n\n" + countdown_line(total_seconds)).strip()

    photo = del_cfg.get("photo")
    try:
        if photo:
            msg = bot.send_photo(chat_id, photo, caption=full_text, caption_entities=entities)
        else:
            msg = bot.send_message(chat_id, full_text or " ", entities=entities)
    except Exception:
        msg = None

    if not msg:
        threading.Timer(total_seconds, delete_many, args=(chat_id, sent_ids)).start()
        return

    interval = 60 if total_seconds > 300 else 15
    state = {"remaining": total_seconds}

    def tick():
        state["remaining"] -= interval
        if state["remaining"] <= 0:
            delete_many(chat_id, sent_ids)
            finalize_deleted_message(chat_id, msg.message_id, code, user, photo)
            return
        new_text = (base_text + "\n\n" + countdown_line(state["remaining"])).strip()
        try:
            if photo:
                bot.edit_message_caption(caption=new_text, chat_id=chat_id, message_id=msg.message_id,
                                          caption_entities=entities)
            else:
                bot.edit_message_text(new_text, chat_id, msg.message_id, entities=entities)
        except Exception:
            pass
...
def build_content_menu():
    ...
    for key in ("welcome", "delete_msg", "resend_msg"):
        lines.append(f"{CONTENT_NUMBERING[key]} <b>{CONTENT_LABELS[key]}</b>")
    lines.append("")
    lines.append("Choisis un élément à modifier :")
    kb = types.InlineKeyboardMarkup()
    for key in ("welcome", "delete_msg", "resend_msg"):
        kb.row(
            types.InlineKeyboardButton(f"{CONTENT_NUMBERING[key]} 🖼️ Image", callback_data=f"cimg:{key}"),
...
def cmd_setdelete(message):
    ...
    bot.reply_to(message, "✏️ Envoie le texte du message de suppression. Astuce : /placeholders")
...
def callback_router(call):
    ...
    if data.startswith("ctxt:"):
        ...
        edit_or_send(chat_id, msg_id, f"✏️ Envoie le texte pour « {CONTENT_LABELS[key]} » (mise en forme conservée). Astuce : /placeholders")
        return
