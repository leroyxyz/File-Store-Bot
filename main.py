...
# ==========================================
# HELPERS - ENTITÉS (gras, italique, citation, code, liens du CONTENU admin)
# ==========================================
...
# ==========================================
# HELPERS - ABONNEMENT OBLIGATOIRE (multi-canaux)
# ==========================================
...
# ==========================================
# PANNEAU RÉGLAGES — MENUS PAR CATÉGORIE
# ==========================================
...
# ==========================================
# Démarrage des flux multi-étapes (commandes texte)
# ==========================================
...
def handle_pending(message):
    ...
    if action == "fs_add_channel":
        ...
        if any(c["channel_id"] == chat_id2 for c in fs_existing.get("channels", [])):
            ...
            return True
        state["data"] = {"channel_id": chat_id2}
        state["action"] = "fs_add_link"
        bot.reply_to(message, "Envoie maintenant le lien d'invitation public de ce canal (https://t.me/...).")
        return True
    ...
    if action == "fs_add_link":
        ...
        try:
            title = bot.get_chat(channel_id).title
        except Exception:
            title = "Canal"
        fs = get_setting("force_sub") or {"enabled": False, "channels": []}
    ...
    if action == "broadcast":
        ...
        if main_link:
            kb = types.InlineKeyboardMarkup()
            kb.row(types.InlineKeyboardButton("➤ Voir plus", url=main_link))
        ...
        for target in user_ids:
            try:
                send_rich_message(target, content, reply_markup=kb)
                sent += 1
            except Exception:
                failed += 1
...
def callback_router(call):
    ...
    if data == "fs_toggle":
        ...
        if not fs.get("channels"):
            ...
            bot.send_message(chat_id, "Transfère un message du canal à rendre obligatoire.")
            return
    ...
    if data == "fs_add_channel_start":
        bot.answer_callback_query(call.id)
        pending[user.id] = {"action": "fs_add_channel"}
        bot.send_message(chat_id, "Transfère un message du canal à ajouter.")
        return
