...
def run_countdown_and_delete(chat_id, sent_ids, code, user, total_minutes):
    """Envoie LE texte personnalisé (menu Contenu ➜ Message de suppression) tel quel, et le
    ré-affiche en boucle avec {time} qui décompte en direct — sans rien ajouter derrière.
    ...
    text, entities = render(total_seconds)
    try:
        if photo:
            msg = bot.send_photo(chat_id, photo, caption=text or None, caption_entities=entities)
        else:
            msg = bot.send_message(chat_id, text or " ", entities=entities)
    except Exception as e:
        print(f"⚠️ [run_countdown_and_delete] Envoi initial échoué : {e!r}")
        msg = None
    ...
    # Le compteur avance minute par minute (comme demandé) : on édite le message toutes les 60s.
    interval = 60
    ...
    def tick():
        ...
        if state["remaining"] <= 0:
            ...
            return
        new_text, new_entities = render(state["remaining"])
        try:
            if photo:
                bot.edit_message_caption(caption=new_text, chat_id=chat_id, message_id=msg.message_id,
                                          caption_entities=new_entities)
            else:
                bot.edit_message_text(new_text, chat_id, msg.message_id, entities=new_entities)
        except Exception as e:
            print(f"⚠️ [run_countdown_and_delete] Édition du compteur échouée : {e!r}")
